...
class Entry(val priority: Int, var data: Map[BaseVariable, Any]) {
  ...
  def merge(entry: Entry): Option[Entry] = {
    ...
    if (!conflict) {
      ...
      var result: Map[BaseVariable, Any] = Map.empty
      for (a <- attributes) {
...
class Table {
  var attributes: Set[BaseVariable] = Set.empty
  ...
  def project(cols: Set[BaseVariable]): Table = {
    val table = new Table
